...
@Properties({
  ...
  @Property(name = "bli.id", value = "BLI2321_bli_buscarfacturafechas")
})
public class BLI2321_bli_buscarfacturafechas implements IBLIExecutor {
  private static final ILogger logger = LogFactory.getLogger(BLI2321_bli_buscarfacturafechas.class);
  ...
  public void execute(DynamicRequest dynamicRequest) {
    ...
    try {
      ...
      mapper.addInputParameter("@i_operacion", SqlType.CHAR, "F");
   	  if (dynamicRequest.getData().get("fechaInicio") != null){
        mapper.addInputParameter("@i_fecha_ini", SqlType.DATETIME, BLIUtils.dateToString(dynamicRequest.getData().get("fechaInicio"), null));
      }
   	  if (dynamicRequest.getData().get("fechaFin") != null){
        mapper.addInputParameter("@i_fecha_fin", SqlType.DATETIME, BLIUtils.dateToString(dynamicRequest.getData().get("fechaFin"), null));
      }
